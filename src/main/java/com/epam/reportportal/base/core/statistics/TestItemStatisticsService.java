...
package com.epam.reportportal.base.core.statistics;
...
/**
 * Service for managing test item statistics directly via SQL, bypassing Hibernate. All operations use advisory locks on
 * launch_id to prevent deadlocks.
 *
...
public interface TestItemStatisticsService {
  ...
  /**
   * Change defect statistics when issue type changes. Decrements old defect type fields and increments new defect type
   * fields by 1 for the item itself, all ancestors (by path) and the launch.
   *
   * @param item         leaf test item
  ...
  /**
   * Subtracts all item's non-zero counters from ancestors and launch, then removes the item's own statistics rows.

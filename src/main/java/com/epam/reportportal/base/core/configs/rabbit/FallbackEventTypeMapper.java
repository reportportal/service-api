...
/**
 * Type mapper that resolves event classes from RabbitMQ message headers. Primary resolution uses fully qualified class
 * name from {@code __TypeId__} header. Falls back to simple class name matching when the class is not found locally,
 * enabling seamless integration with external services using different package structures.
 */
@Slf4j
@Component
public class FallbackEventTypeMapper implements Jackson2JavaTypeMapper {
  ...
  /**
   * Returns the type precedence strategy. Uses {@link TypePrecedence#TYPE_ID} to resolve types from {@code __TypeId__}
   * header, consistent with {@link org.springframework.amqp.support.converter.DefaultJackson2JavaTypeMapper}.
   *
  ...
  /**
   * Returns inferred type from message properties. Not used when {@link TypePrecedence#TYPE_ID} is set. Defined by

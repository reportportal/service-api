...
public interface TestItemRepository extends ReportPortalRepository<TestItem, Long>,
  ...
  /**
   * Interrupts all
   * {@link com.epam.reportportal.base.infrastructure.persistence.entity.enums.StatusEnum#IN_PROGRESS}
   * children items of the launch with provided launchId. Sets them
   * {@link com.epam.reportportal.base.infrastructure.persistence.entity.enums.StatusEnum#INTERRUPTED}
   * status
   *
   * @param launchId Launch id
   */
  ...
  @Query("""
      SELECT new com.epam.reportportal.base.core.tms.dto.CountOfChildTestItemsByParentId(ti.parentId, COUNT(ti.itemId))
      FROM TestItem ti
  ...
  @Query(value =
      "WITH RECURSIVE ids AS ("
          + "  SELECT item_id, parent_id FROM test_item WHERE item_id IN :itemIds AND launch_id = :launchId "
          + "  UNION ALL "
          + "  SELECT ti.item_id, ti.parent_id FROM test_item ti JOIN ids ON ti.item_id = ids.parent_id"
          + ") "
          + "SELECT item_id FROM ids",
      nativeQuery = true)

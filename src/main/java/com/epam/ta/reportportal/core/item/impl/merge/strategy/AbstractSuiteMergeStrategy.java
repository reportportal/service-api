...
public abstract class AbstractSuiteMergeStrategy implements MergeStrategy {
	...
	private TestItem moveAllChildTestItems(TestItem itemTarget, TestItem itemSource) {
		for (TestItem childItem : testItemRepository.selectAllDescendants(itemSource.getItemId())) {
			childItem.getItemStructure().setLaunch(itemTarget.getItemStructure().getLaunch());
			childItem.getItemStructure().setParent(itemTarget.getItemStructure());

			setLaunchRefForChildren(childItem, itemTarget.getItemStructure().getLaunch());
	...
	private void setLaunchRefForChildren(TestItem testItemParent, Launch launch) {
		...
		for (TestItem child : childItems) {
			child.getItemStructure().setLaunch(launch);
	...
	private TestItem updateTime(TestItem target, TestItem source) {
		...
		LocalDateTime targetEndTime = target.getItemStructure().getItemResults().getEndTime();
		LocalDateTime sourceEndTime = source.getItemStructure().getItemResults().getEndTime();
		target.getItemStructure().getItemResults().setEndTime(targetEndTime.isAfter(sourceEndTime) ? targetEndTime : sourceEndTime);

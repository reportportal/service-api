...
import com.epam.reportportal.base.core.item.repository.RetryRepository;
...
public class RetryHandlerImpl implements RetryHandler {
  ...
  /**
   * Explicit mode: {@code newTry} is a new attempt of the specific item with {@code uuid = retryOf}. The target is
   * demoted and {@code newTry} becomes the main item. Falls back to implicit mode if the target UUID is not found.
   */
  ...
  /**
   * Implicit mode: discovers all active items sharing {@code newTry.uniqueId} and {@code newTry.parentId}, picks the
   * latest by {@code start_time} as the winner, and demotes the rest.
   */

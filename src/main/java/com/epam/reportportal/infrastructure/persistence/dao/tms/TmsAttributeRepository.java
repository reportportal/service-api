...
public interface TmsAttributeRepository extends ReportPortalRepository<TmsAttribute, Long> {
  ...
  @Query("SELECT DISTINCT a.key FROM TmsAttribute a WHERE a.project.id = :projectId AND a.key LIKE %:search%")
  List<String> findDistinctKeysByProjectIdAndKeyLike(@Param("projectId") Long projectId,
      @Param("search") String search);

  @Query("SELECT DISTINCT a.value FROM TmsAttribute a WHERE a.project.id = :projectId AND a.value IS NOT NULL AND a.value LIKE %:search%")
  List<String> findDistinctValuesByProjectIdAndValueLike(@Param("projectId") Long projectId,
      @Param("search") String search);

...
class FileStorageControllerTest extends BaseMvcTest {
  ...
  void userPhoto() throws Exception {
    ...
    mockMvc.perform(get("/v1/data/default_personal/userphoto?id=default").with(
  ...
  void getUserPhotoByLoginNegative() throws Exception {
    mockMvc.perform(get("/v1/data/superadmin_personal/userphoto?id=superadmin").with(
            token(oAuthHelper.getSuperadminToken())))
        .andExpect(status().isOk());
  }
  ...
  void getNotExistUserPhoto() throws Exception {
    mockMvc.perform(
            get("/v1/data/userphoto?id=not_exist").with(token(oAuthHelper.getSuperadminToken())))

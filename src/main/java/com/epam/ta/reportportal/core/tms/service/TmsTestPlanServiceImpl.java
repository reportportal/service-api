...
public class TmsTestPlanServiceImpl implements TmsTestPlanService {
  ...
  @Override
  @Transactional
  public TmsTestPlanRS duplicate(long projectId, Long testPlanId) {
    // Find the original test plan
    var originalTestPlan = testPlanRepository.findByIdAndProjectId(testPlanId, projectId)
        .orElseThrow(() -> new ReportPortalException(
            NOT_FOUND, TMS_TEST_PLAN_NOT_FOUND_BY_ID.formatted(testPlanId, projectId))
        );

    // Duplicate the test plan entity with "-copy" postfix
    var duplicatedTestPlan = tmsTestPlanMapper.duplicateTestPlan(originalTestPlan);
    testPlanRepository.save(duplicatedTestPlan);

    // Duplicate test plan attributes
    if (originalTestPlan.getAttributes() != null && !originalTestPlan.getAttributes().isEmpty()) {
      tmsTestPlanAttributeService.createTestPlanAttributes(
          duplicatedTestPlan,
          originalTestPlan.getAttributes().stream()
              .map(attr -> {
                var attrRQ = new com.epam.ta.reportportal.core.tms.dto.TmsTestPlanAttributeRQ();
                attrRQ.setId(attr.getAttribute().getId());
                attrRQ.setValue(attr.getValue());
                return attrRQ;
              })
              .toList()
      );
    ...
    // Get all test case IDs associated with the original test plan
    var originalTestCaseIds = tmsTestPlanTestCaseRepository.findTestCaseIdsByTestPlanId(testPlanId);

    if (!originalTestCaseIds.isEmpty()) {
      // Duplicate all test cases using the existing duplicate functionality
      var duplicateRequest = com.epam.ta.reportportal.core.tms.dto.batch.BatchDuplicateTestCasesRQ.builder()
          .testCaseIds(originalTestCaseIds)
          .build();

      var duplicatedTestCases = tmsTestCaseService.duplicate(projectId, duplicateRequest);

      // Add all duplicated test cases to the new test plan
      var duplicatedTestCaseIds = duplicatedTestCases.stream()
          .map(com.epam.ta.reportportal.core.tms.dto.TmsTestCaseRS::getId)
          .toList();

      addTestCasesToPlan(projectId, duplicatedTestPlan.getId(), duplicatedTestCaseIds);
    }

    // Return the duplicated test plan with statistics
    return tmsTestPlanMapper.convertTmsTestPlanWithStatisticToRS(
        tmsTestPlanExecutionService.enrichWithStatistics(duplicatedTestPlan)
    );
  }
}
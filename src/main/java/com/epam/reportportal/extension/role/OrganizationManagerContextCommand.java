...
public abstract class OrganizationManagerContextCommand<T> extends OrganizationMemberContextCommand<T> {
  ...
  protected void validateOrganizationRole(OrganizationUser organizationUser) {
    if (!organizationUser.getOrganizationRole().sameOrHigherThan(OrganizationRole.MANAGER)) {
      throw new ReportPortalException(ErrorType.ACCESS_DENIED, "Organization Manager role required.");
    }
  }

...
public class TmsTestPlanServiceImpl implements TmsTestPlanService {
  ...
  public TmsTestPlanRS duplicate(long projectId, Long testPlanId) {
    ...
    if (!originalTestCaseIds.isEmpty()) {
      // Duplicate all test cases using existing service method
      ...
      // Add duplicated test cases to the new test plan
      var duplicatedTestCaseIds = duplicatedTestCases.stream()
          .map(testCase -> testCase.getId())
          .toList();

      for (var duplicatedTestCaseId : duplicatedTestCaseIds) {
        addTestCaseToTestPlan(duplicatedTestPlan.getId(), duplicatedTestCaseId);
      }
    }

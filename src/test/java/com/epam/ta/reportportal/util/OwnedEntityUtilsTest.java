...
class OwnedEntityUtilsTest {
  ...
  private ReportPortalUser.ProjectDetails adminProjectDetails;

  private ReportPortalUser projectManagerUser;
  private ReportPortalUser.ProjectDetails projectManagerProjectDetails;

  private ReportPortalUser memberUser;
  private ReportPortalUser.ProjectDetails memberProjectDetails;

  @BeforeEach
  void setUp() {
    adminUser = mock(ReportPortalUser.class);
    adminProjectDetails = mock(ReportPortalUser.ProjectDetails.class);
    when(adminUser.getUserRole()).thenReturn(UserRole.ADMINISTRATOR);

    projectManagerUser = mock(ReportPortalUser.class);
    projectManagerProjectDetails = mock(ReportPortalUser.ProjectDetails.class);
    when(projectManagerUser.getUserRole()).thenReturn(UserRole.USER);
    when(projectManagerProjectDetails.getProjectRole()).thenReturn(ProjectRole.PROJECT_MANAGER);

    memberUser = mock(ReportPortalUser.class);
    memberProjectDetails = mock(ReportPortalUser.ProjectDetails.class);
    when(memberUser.getUserRole()).thenReturn(UserRole.USER);
    when(memberProjectDetails.getProjectRole()).thenReturn(ProjectRole.MEMBER);
  }

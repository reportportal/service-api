...
public class GetProjectInfoHandlerImpl implements GetProjectInfoHandler {
  ...
  private static final Predicate<ActivityAction> ACTIVITIES_PROJECT_FILTER = it ->
      it == UPDATE_DEFECT || it == DELETE_DEFECT
  ...
  public ProjectInfoResource getProjectInfo(String projectName, String interval) {
    ...
    Filter filter = Filter.builder()
        .withTarget(ProjectInfo.class)
        .withCondition(
            FilterCondition.builder().eq(CRITERIA_PROJECT_NAME, project.getName()).build())
        .build();

    Page<ProjectInfo> result = projectRepository.findProjectInfoByFilter(filter,
        Pageable.unpaged());
    ProjectInfoResource projectInfoResource = ProjectSettingsConverter.TO_PROJECT_INFO_RESOURCE.apply(
        result.get()
            .findFirst()
            .orElseThrow(() -> new ReportPortalException(PROJECT_NOT_FOUND, projectName)));
    ...
    Map<String, Integer> countPerUser = launchRepository.countLaunchesGroupedByOwner(
        project.getId(),
  ...
  private Map<String, List<ActivityResource>> getActivities(Project project,
    ...
    Filter filter = new Filter(Activity.class,
        Lists.newArrayList(new FilterCondition(IN, false, value, CRITERIA_ACTION),
    ...
    return Collections.singletonMap(RESULT,
        activities.stream()
            .map(a -> ofNullable(a.getUserId()).map(

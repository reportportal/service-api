...
package com.epam.reportportal.core.analyzer.auto.client;

import com.epam.reportportal.core.analyzer.auto.client.impl.AnalyzerUtils;
 ...
 * <p>
 * To define that service indexes/collecting data it should be indicated by tag {@link AnalyzerUtils#ANALYZER_INDEX}
 * with <code>true</code> in metadata. If tag is not provided it is <code>false</code> by default
 * <p>
 * Items are analyzed in order of priority specified in tag * {@link AnalyzerUtils#ANALYZER_PRIORITY} in metadata. If
 * priority is not provided service gets the lowest one. If several analyzers provided different issues for one item, it
 * would be overwritten with results of more priority service.
 *

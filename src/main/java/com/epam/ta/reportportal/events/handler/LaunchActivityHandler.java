...
public class LaunchActivityHandler {
	...
	public void onLaunchStart(LaunchStartedEvent event) {
		...
		if (Mode.DEBUG != event.getLaunch().getMode()) {
			...
			Activity activityLog = new ActivityBuilder().addUserRef(launch.getUserRef()).addProjectRef(launch.getProjectRef().toLowerCase())
					.addActionType(START_LAUNCH).addObjectType(ActivityObjectType.LAUNCH).addLoggedObjectRef(launch.getId()).addObjectName(name).build();
	...
	public void onDeleteLaunch(LaunchDeletedEvent event) {
		...
		if (null != launch && launch.getMode() == Mode.DEFAULT) {
			...
			Activity activity = new ActivityBuilder().addUserRef(event.getDeletedBy()).addProjectRef(event.getLaunch().getProjectRef())
					.addActionType(DELETE_LAUNCH).addObjectType(ActivityObjectType.LAUNCH).addLoggedObjectRef(launch.getId()).addObjectName(name).build();
	...
	private void afterLaunchFinished(Launch launch, String finishedBy) {
		if (launch.getMode() != Mode.DEBUG) {
			String name = launch.getName() + DELIMITER + launch.getNumber();
			Activity activityLog = new ActivityBuilder().addUserRef(finishedBy).addProjectRef(launch.getProjectRef()).addActionType(FINISH_LAUNCH)
					.addObjectType(ActivityObjectType.LAUNCH).addLoggedObjectRef(launch.getId()).addObjectName(name).build();
			activityRepository.save(activityLog);
		}
	}

...
public class PluginLoaderImpl implements PluginLoader {
  ...
  public boolean load(IntegrationType integrationType) {
    ...
    return ofNullable(integrationType.getDetails()).map(IntegrationTypeDetails::getDetails)
        .map(details -> {
          final String pluginId = integrationType.getName();
          final String fileName = getPropertyValue(pluginId, details,
              IntegrationTypeProperties.FILE_NAME);
          final Path pluginPath = Paths.get(pluginsDir, fileName);
          if (Files.notExists(pluginPath)) {
            loadPlugin(details, pluginId, fileName, pluginPath);
          }

          return pluginBox.loadPlugin(pluginPath).flatMap(pluginBox::getPluginById)
              .map(pluginWrapper -> {
                if (PluginState.STARTED == pluginBox.startUpPlugin(pluginWrapper)) {
                  final Optional<ExtensionPoint> extensionPoint = pluginBox.getInstance(pluginId,
  ...
  private void loadPlugin(Map<String, Object> details, String pluginId, String fileName,
      Path pluginPath) {
    final String resourcesDir = getPropertyValue(pluginId, details,
        IntegrationTypeProperties.RESOURCES_DIRECTORY);
    final String fileId = getPropertyValue(pluginId, details, IntegrationTypeProperties.FILE_ID);
    final PluginPathInfo pluginPathInfo = new PluginPathInfo(pluginPath, Paths.get(resourcesDir),
        fileName, fileId);
    pluginFileManager.download(pluginPathInfo);
  }

  private String getPropertyValue(String pluginId, Map<String, Object> details,
      IntegrationTypeProperties property) {
  ...
  public boolean unload(IntegrationType integrationType) {
    return pluginBox.getPluginById(integrationType.getName()).map(pluginBox::unloadPlugin)
        .orElse(Boolean.TRUE);
  }
}
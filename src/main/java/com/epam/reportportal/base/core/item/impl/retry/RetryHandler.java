...
public interface RetryHandler {
  ...
  /**
   * Links {@code lastTry} and {@code previousTry} through the retry mechanism. Compares {@code start_time} to decide
   * which item becomes the "main" (keeps path/launch_id/statistics) and which becomes the "retry" (path=NULL,
   * launch_id=NULL, retry_of=main). Deletes the loser's statistics and issue, then updates the rows accordingly.
   *
  ...
  /**
   * Finishes all in-progress retry items that point to the given main item ({@code retry_of = itemId}). Called when the
   * main item itself finishes.
   *

...
import com.epam.reportportal.infrastructure.persistence.commons.querygen.ProjectFilter;
...
class DemoDashboardsService {
  ...
  @Autowired
  public DemoDashboardsService(UserFilterRepository userFilterRepository,
      DashboardRepository dashboardRepository, DashboardWidgetRepository dashboardWidgetRepository,
      WidgetRepository widgetRepository, ProjectRepository projectRepository,
    ...
    try {
      ...
      List<Widget> widgets = objectMapper.readValue(resource.getURL(), type).stream().map(it -> {
        final WidgetBuilder widgetBuilder =
            new WidgetBuilder().addWidgetRq(it).addProject(projectId).addOwner(user.getUsername());
        final WidgetType widgetType = WidgetType.findByName(it.getWidgetType()).orElseThrow(
            () -> new ReportPortalException(ErrorType.UNABLE_TO_CREATE_WIDGET,
                "Widget type '" + it.getWidgetType() + "' does not exists"
            ));
        if (!WidgetType.FLAKY_TEST_CASES.equals(widgetType) || !WidgetType.TOP_TEST_CASES.equals(
            widgetType)) {
  ...
  private UserFilter createDemoFilter(ReportPortalUser user, Project project) {
    List<UserFilter> existedFilterList = userFilterRepository.findByFilter(ProjectFilter.of(
        Filter.builder().withTarget(UserFilter.class).withCondition(
            FilterCondition.builder().withCondition(Condition.EQUALS)
                .withSearchCriteria(CRITERIA_NAME).withValue(FILTER_NAME).build()).build(),
        project.getId()
    ...
    userFilter.setFilterCondition(Sets.newHashSet(
        FilterCondition.builder().withSearchCriteria(CRITERIA_COMPOSITE_ATTRIBUTE)
            .withCondition(Condition.HAS).withValue("demo").build()));
  ...
  private Dashboard createDemoDashboard(List<Widget> widgets, ReportPortalUser user,
    ...
    dashboard.addWidget(
        createDashboardWidget(user.getUsername(), dashboard, widgets.get(0), 0, 0, 6, 5));
    dashboard.addWidget(
        createDashboardWidget(user.getUsername(), dashboard, widgets.get(1), 6, 0, 6, 5));
    dashboard.addWidget(
        createDashboardWidget(user.getUsername(), dashboard, widgets.get(2), 0, 5, 7, 5));
    dashboard.addWidget(
        createDashboardWidget(user.getUsername(), dashboard, widgets.get(3), 7, 5, 5, 5));
    dashboard.addWidget(
        createDashboardWidget(user.getUsername(), dashboard, widgets.get(4), 0, 10, 5, 5));
    dashboard.addWidget(
        createDashboardWidget(user.getUsername(), dashboard, widgets.get(5), 5, 10, 7, 5));
    dashboard.addWidget(
        createDashboardWidget(user.getUsername(), dashboard, widgets.get(6), 0, 15, 6, 5));
    dashboard.addWidget(
        createDashboardWidget(user.getUsername(), dashboard, widgets.get(7), 6, 15, 6, 5));
    dashboard.addWidget(
        createDashboardWidget(user.getUsername(), dashboard, widgets.get(8), 0, 20, 12, 4));
    dashboard.addWidget(
        createDashboardWidget(user.getUsername(), dashboard, widgets.get(9), 0, 24, 7, 5));
    dashboard.addWidget(
        createDashboardWidget(user.getUsername(), dashboard, widgets.get(10), 7, 24, 5, 5));
    dashboard.addWidget(
        createDashboardWidget(user.getUsername(), dashboard, widgets.get(11), 0, 29, 12, 4));

    return dashboard;
  }

  private DashboardWidget createDashboardWidget(String owner, Dashboard dashboard, Widget widget,

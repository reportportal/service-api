...
public interface TmsStepRepository extends ReportPortalRepository<TmsStep, Long> {

  @Modifying
  @Query("DELETE FROM TmsStep s WHERE s.manualScenario.id IN ("
  ...
  @Modifying
  @Query(value = "DELETE FROM tms_step s "
      + "WHERE s.manual_scenario_id IN ("
      ...
      + ") ",
      nativeQuery = true)
  void deleteStepsByTestFolderId(@Param("projectId") Long projectId,

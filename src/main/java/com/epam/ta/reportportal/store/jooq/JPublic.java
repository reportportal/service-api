...
public class JPublic extends SchemaImpl {
	...
	private static final long serialVersionUID = 1518306590;
	...
	private final List<Sequence<?>> getSequences0() {
		return Arrays.<Sequence<?>>asList(Sequences.BUG_TRACKING_SYSTEM_ID_SEQ, Sequences.DASHBOARD_ID_SEQ,
				Sequences.DEFECT_FIELD_ALLOWED_VALUE_ID_SEQ, Sequences.DEFECT_FORM_FIELD_ID_SEQ, Sequences.ISSUE_TYPE_ID_SEQ,
				Sequences.ITEM_TAG_ID_SEQ, Sequences.LAUNCH_ID_SEQ, Sequences.LAUNCH_TAG_ID_SEQ, Sequences.LOG_ID_SEQ,
				Sequences.OAUTH_REGISTRATION_SCOPE_ID_SEQ, Sequences.PROJECT_CONFIGURATION_ID_SEQ,
				Sequences.PROJECT_EMAIL_CONFIGURATION_ID_SEQ, Sequences.PROJECT_ID_SEQ, Sequences.SERVER_SETTINGS_ID_SEQ,
				Sequences.TEST_ITEM_ITEM_ID_SEQ, Sequences.TICKET_ID_SEQ, Sequences.USERS_ID_SEQ, Sequences.WIDGET_ID_SEQ
		);
	}
	...
	private final List<Table<?>> getTables0() {
		return Arrays.<Table<?>>asList(JBugTrackingSystem.BUG_TRACKING_SYSTEM, JDashboard.DASHBOARD, JDashboardWidget.DASHBOARD_WIDGET,
				JDefectFieldAllowedValue.DEFECT_FIELD_ALLOWED_VALUE, JDefectFormField.DEFECT_FORM_FIELD, JIssue.ISSUE,
				JIssueTicket.ISSUE_TICKET, JIssueType.ISSUE_TYPE, JIssueTypeProjectConfiguration.ISSUE_TYPE_PROJECT_CONFIGURATION,
				JItemTag.ITEM_TAG, JLaunch.LAUNCH, JLaunchTag.LAUNCH_TAG, JLog.LOG, JOauthAccessToken.OAUTH_ACCESS_TOKEN,
				JOauthRegistration.OAUTH_REGISTRATION, JOauthRegistrationScope.OAUTH_REGISTRATION_SCOPE, JProject.PROJECT,
				JProjectConfiguration.PROJECT_CONFIGURATION, JProjectEmailConfiguration.PROJECT_EMAIL_CONFIGURATION,
				JProjectUser.PROJECT_USER, JServerSettings.SERVER_SETTINGS, JTestItem.TEST_ITEM, JTestItemResults.TEST_ITEM_RESULTS,
				JTestItemStructure.TEST_ITEM_STRUCTURE, JTicket.TICKET, JUsers.USERS, JWidget.WIDGET
		);
	}
	...
	private final List<UDT<?>> getUDTs0() {
		return Arrays.<UDT<?>>asList(JParameter.PARAMETER);
	}
}

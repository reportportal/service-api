...
public class GetLogHandlerImpl implements GetLogHandler {
  ...
  public Iterable<?> getNestedItems(Long parentId, ReportPortalUser.ProjectDetails projectDetails,
    ...
    boolean isLogMessageFilter = queryable.getFilterConditions().stream()
        .anyMatch(condition -> ((FilterCondition) condition).getSearchCriteria().equals("message"));

    Page<NestedItem> nestedItems;

    if (isLogMessageFilter) {
      nestedItems = logService.findNestedByLogMessageFilter(parentId, parentItem.getPath(),
          projectDetails.getProjectId(), launch.getId(), queryable, pageable
      );
    } else {
      nestedItems = logRepository.findNestedItems(parentId, excludeEmptySteps,

...
class TmsManualScenarioAttributeServiceImplTest {
  ...
  @Test
  void shouldPatchAttributesAddingNewOnes() {
    // Given
    var existingAttributes = createExistingManualScenarioAttributes();
    manualScenario.setAttributes(existingAttributes);

    var attributeRQ = createAttributeRQWithKeyAndValue("new-key", "new-value");
    var attributeRQs = List.of(attributeRQ);

    var tmsAttribute = createTmsAttribute(3L, "new-key", "new-value");

    when(tmsAttributeService.findOrCreateAttribute(PROJECT_ID, "new-key", "new-value"))
        .thenReturn(tmsAttribute);

    // When
    tmsManualScenarioAttributeService.patchAttributes(PROJECT_ID, manualScenario, attributeRQs);

    // Then
    verify(tmsAttributeService).findOrCreateAttribute(PROJECT_ID, "new-key", "new-value");
    verify(tmsManualScenarioAttributeRepository).saveAll(attributesCaptor.capture());

    var savedAttributes = attributesCaptor.getValue();
    assertThat(savedAttributes).hasSize(1);
    assertThat(manualScenario.getAttributes()).hasSize(3); // 2 existing + 1 new
  }
  ...
  void shouldPatchAttributesWhenNoExistingAttributes() {
    ...
    var attributeRQ = createAttributeRQWithId(1L);
    var attributeRQs = List.of(attributeRQ);

    var tmsAttribute = createTmsAttribute(1L, "key1", "value1");

    when(tmsAttributeService.getEntityById(PROJECT_ID, 1L)).thenReturn(tmsAttribute);
  ...
  void shouldNotPatchAttributesWhenListIsEmpty() {
    // When
    tmsManualScenarioAttributeService.patchAttributes(PROJECT_ID, manualScenario,
        Collections.emptyList());
  ...
  @Test
  void shouldNotPatchAttributesWhenListIsNull() {
    // When
    tmsManualScenarioAttributeService.patchAttributes(PROJECT_ID, manualScenario, null);
  ...
  @Test
  void shouldPatchAttributesMixedIdAndKeyValue() {
    // Given
    var existingAttributes = createExistingManualScenarioAttributes();
    manualScenario.setAttributes(existingAttributes);

    var attributeRQ1 = createAttributeRQWithId(10L);
    var attributeRQ2 = createAttributeRQWithKeyAndValue("patch-key", "patch-value");
    var attributeRQs = List.of(attributeRQ1, attributeRQ2);

    var tmsAttribute1 = createTmsAttribute(10L, "key10", "value10");
    var tmsAttribute2 = createTmsAttribute(11L, "patch-key", "patch-value");

    when(tmsAttributeService.getEntityById(PROJECT_ID, 10L)).thenReturn(tmsAttribute1);
    when(tmsAttributeService.findOrCreateAttribute(PROJECT_ID, "patch-key", "patch-value"))
        .thenReturn(tmsAttribute2);
    ...
    verify(tmsAttributeService).findOrCreateAttribute(PROJECT_ID, "patch-key", "patch-value");
    verify(tmsManualScenarioAttributeRepository).saveAll(attributesCaptor.capture());

    var savedAttributes = attributesCaptor.getValue();
    assertThat(savedAttributes).hasSize(2);
    assertThat(manualScenario.getAttributes()).hasSize(4); // 2 existing + 2 new
  }

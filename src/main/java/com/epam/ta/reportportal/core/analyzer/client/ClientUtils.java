...
public final class ClientUtils {
	...
	static final ToIntFunction<ServiceInstance> SERVICE_PRIORITY = it -> {
		try {
			...
			LOGGER.warn("Incorrect specification of tag '{}' for service '{}'. Using the lowest priority",
					ANALYZER_PRIORITY,
					it.getMetadata().get(ANALYZER_KEY),
					e
			);
	...
	static final Predicate<ServiceInstance> SUPPORT_INDEX = it -> {
		try {
			...
			LOGGER.warn("Incorrect specification of tag '{}' for service '{}'. Using 'false' as default value.",
					ANALYZER_INDEX,
					it.getMetadata().get(ANALYZER_KEY),
					e
			);

...
public class DataStorageExceptionTest {
    ...
    private final String MESSAGE = "DataStorageException appeared";
    private final long EXPECTED_SERIAL_VERSION_UID =
            (long) ReflectionTestUtils.getField(DataStorageException.class, "serialVersionUID");

    @Test(expected = DataStorageException.class)
    public void dataStorageExceptionLocalTest() {
    ...
    @Test(expected = DataStorageException.class)
    public void dataStorageExceptionWithThrowableLocalTest() {
        try {
            throw new DataStorageException(MESSAGE, new Throwable(MESSAGE));
        } catch (DataStorageException e) {
            assertEquals(MESSAGE, e.getMessage());

...
import com.epam.reportportal.base.core.analyzer.auto.client.impl.AnalyzerUtils;
 ...
 * <p>
 * Analysis requests are dispatched to all analyzers simultaneously and asynchronously. Priority is no longer
 * resolved: every result delivered to the reply queue is treated as the actual one and overwrites the previous issue
 * for the item (last-write-wins).
 *
...
public interface AnalyzerServiceClient {
  ...
  /**
   * Dispatches the launch for analysis to every analyzer asynchronously (fire-and-forget). The request is sent to
   * all analyzer exchanges simultaneously; results are delivered back through the reply queue and handled by a
   * dedicated listener.
   *
   * @param rq Launch
   */

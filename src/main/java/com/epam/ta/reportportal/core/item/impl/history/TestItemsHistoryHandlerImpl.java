...
public class TestItemsHistoryHandlerImpl implements TestItemsHistoryHandler {
	...
	public List<TestItemHistoryElement> getItemsHistory(ReportPortalUser.ProjectDetails projectDetails, ReportPortalUser user,
		...
		Page<Long> testItems = getTestItemHandler.getTestItemsIds(projectDetails, user, filter, pageable, launchId, filterId, launchesLimit);

		List<Long> itemIds = new ArrayList<>(testItems.getContent());
	...
	private void validateItems(List<TestItem> itemsForHistory, List<Long> ids, Long projectId) {
	...
	private TestItemHistoryElement buildHistoryElement(Launch launch, List<TestItem> testItems) {
		...
		if (testItems != null) {
			Map<Long, Map<Long, String>> pathNamesMapping = testItemRepository.selectPathNames(testItems.stream().map(TestItem::getItemId).collect(toList()));

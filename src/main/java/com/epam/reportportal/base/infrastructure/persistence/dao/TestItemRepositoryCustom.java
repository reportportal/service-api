...
public interface TestItemRepositoryCustom extends FilterableRepository<TestItem> {
   ...
   *
   * @param filter {@link Queryable} with {@link FilterTarget#TEST_ITEM_TARGET}
   ...
   * @param launchFilter     {@link Queryable} with {@link FilterTarget#LAUNCH_TARGET}
   * @param testItemFilter   {@link Queryable} with {@link FilterTarget#TEST_ITEM_TARGET}
  ...
  /**
   * Loads items {@link TestItemHistory} - {@link TestItem} executions from the whole {@link Project} grouped by
  ...
  /**
   * Loads items {@link TestItemHistory} - {@link TestItem} executions from the {@link Project} with provided
   * `projectId` and {@link Launch#getName()} equal to the provided `launchName` value. Result is grouped by
  ...
  /**
   * Loads items {@link TestItemHistory} - {@link TestItem} executions from the {@link Project} with provided
   * `projectId` and {@link Launch} which IDs are in provided `launchIds`. Result is grouped by
   * {@link TestItem#getTestCaseHash()} and is ordered by {@link TestItem#getStartTime()} `DESCENDING` within group. Max
   * group size equals to the provided `historyDepth` value.
   *
   * @param filter       {@link Queryable}
   ...
   * {@link FilterTarget#LAUNCH_TARGET} and {@link FilterTarget#TEST_ITEM_TARGET}
   *
   * @param isLatest         Flag for retrieving only latest launches
   ...
   * @param endTime {@link TestItemResults#endTime}
  ...
  /**
   * Select {@link NestedStep} entities by provided 'IDs' with {@link NestedStep#attachmentsCount} of the {@link Log}
   * entities of all descendants for each {@link NestedStep} and {@link NestedStep#hasContent} flag to check whether
   * entity is a last one in the descendants tree or there are {@link Log} or {@link NestedStep} entities exist under
   * it
   *
   * @param ids       {@link Collection} of the {@link TestItem#itemId}
   * @param logFilter {@link Queryable} with {@link Log} target, to evaluate 'hasContent' flag and   attachments count

...
import static com.epam.reportportal.base.core.hierarchy.AbstractFinishHierarchyHandler.ATTRIBUTE_KEY_STATUS;
import static com.epam.reportportal.base.core.hierarchy.AbstractFinishHierarchyHandler.ATTRIBUTE_VALUE_INTERRUPTED;
...
import static com.epam.reportportal.base.infrastructure.persistence.entity.enums.StatusEnum.fromValue;
import static com.epam.reportportal.base.infrastructure.persistence.entity.enums.TestItemIssueGroup.NOT_ISSUE_FLAG;
...
import static com.epam.reportportal.base.infrastructure.rules.exception.ErrorType.AMBIGUOUS_TEST_ITEM_STATUS;
import static com.epam.reportportal.base.infrastructure.rules.exception.ErrorType.FINISH_ITEM_NOT_ALLOWED;
...
import com.epam.reportportal.base.core.events.domain.item.TestItemFinishedEvent;
...
class FinishTestItemHandlerImpl implements FinishTestItemHandler {
  ...
  FinishTestItemHandlerImpl(TestItemRepository testItemRepository,
      ...
      ApplicationEventPublisher eventPublisher, LaunchRepository launchRepository, RetryHandler retryHandler,
      ExternalTicketHandler externalTicketHandler, TestItemAttributeHandlerService testItemAttributeHandlerService,
    ...
    if (testItemResults.getStatus() == IN_PROGRESS) {
      ...
      if (testItem.getRetryOf() == null) {
        ...
        if (testItem.isHasRetries()) {
          retryHandler.finishRetries(testItem, JStatusEnum.valueOf(actualStatus.name()),
  ...
  /**
   * Retry flow on item finish: 1. Guard — only leaf items that are not already part of a retry chain 2. Resolve
   * previousTryId — either from explicit {@code retryOf} UUID or by searching 3. Link previousTry ↔ newTry via
   * {@code handle_retry} (SQL decides who becomes main)
   *

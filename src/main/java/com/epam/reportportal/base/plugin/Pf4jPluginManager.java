...
import com.epam.reportportal.base.infrastructure.persistence.entity.plugin.PluginFileExtension;
 ...
 * {@link com.epam.reportportal.base.job.CleanOutdatedPluginsJob} to prevent the removing of the plugins that are still
 * being processed within the database transaction with {@link IntegrationType} in uncommitted state
 */
public class Pf4jPluginManager implements Pf4jPluginBox {
  ...
  public IntegrationType uploadPlugin(final String uploadedPluginName,
    ...
    return ofNullable(pluginManager.loadPlugin(Paths.get(pluginsTempDir, uploadedPluginName)))
        ...
        .orElseThrow(() -> {
          ...
          return new ReportPortalException(ErrorType.PLUGIN_UPLOAD_ERROR,
   ...
   *                 {@link com.epam.reportportal.base.plugin.Pf4jPluginManager#uploadingPlugins}
   * @param path     Full path to the plugin file
   * @see com.epam.reportportal.base.plugin.Pf4jPluginManager
   */

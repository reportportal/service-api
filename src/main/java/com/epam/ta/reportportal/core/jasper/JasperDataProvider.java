...
public class JasperDataProvider {
	...
	public List<TestItemPojo> getReportSource(Launch launch) {
		...
		List<TestItem> ownedItems = testItemRepository.findTestItemsByLaunchId(launch.getId());
	...
	private static TestItem adjustName(TestItem input) {
		...
		StringBuilder sb = new StringBuilder(StringUtils.repeat(PREFIX, input.getPath().split(".").length));

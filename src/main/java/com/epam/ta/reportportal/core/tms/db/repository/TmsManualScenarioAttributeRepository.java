...
public interface TmsManualScenarioAttributeRepository extends
    ReportPortalRepository<TmsManualScenarioAttribute, TmsManualScenarioAttributeId> {

  @Modifying
  @Query("DELETE FROM TmsManualScenarioAttribute msa WHERE msa.id.manualScenarioId IN ("
  ...
  @Modifying
  @Query(value = "DELETE FROM tms_manual_scenario_attribute msa "
      + "WHERE msa.manual_scenario_id IN ("
      ...
      + ") ",
      nativeQuery = true)
  void deleteManualScenarioAttributesByTestFolderId(@Param("projectId") Long projectId,

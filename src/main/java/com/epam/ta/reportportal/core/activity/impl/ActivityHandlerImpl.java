...
public class ActivityHandlerImpl implements ActivityHandler {
  ...
  @Autowired
  public ActivityHandlerImpl(ActivityRepository activityRepository,
  ...
  @Override
  public Iterable<ActivityResource> getActivitiesHistory(
      ReportPortalUser.ProjectDetails projectDetails, Filter filter,
  ...
  @Override
  public ActivityResource getActivity(ReportPortalUser.ProjectDetails projectDetails,
      Long activityId) {
    Activity activity = activityRepository.findById(activityId)
        .orElseThrow(() -> new ReportPortalException(ACTIVITY_NOT_FOUND, activityId));
    expect(projectDetails.getProjectId(), Predicate.isEqual(activity.getProjectId())).verify(
        ACCESS_DENIED,
  ...
  public Iterable<ActivityResource> getItemActivities(
    ...
    Launch launch = launchRepository.findById(testItem.getLaunchId())
        .orElseThrow(
            () -> new ReportPortalException(ErrorType.LAUNCH_NOT_FOUND, testItem.getLaunchId()));
    expect(projectDetails.getProjectId(), Predicate.isEqual(launch.getProjectId())).verify(
        ACCESS_DENIED,
  ...
  /**
   * Build {@link Filter} to search for {@link Activity} with
   * {@link Activity.ActivityEntityType#PATTERN} entity and {@link ActivityAction#PATTERN_MATCHED}
   * action conditions of the {@link TestItem} with provided 'itemId'
   *
   * @param filterTarget {@link FilterTarget}
   * @param itemId       {@link Activity#objectId}
   * @return {@link Filter} with {@link Activity.ActivityEntityType#PATTERN},
   * {@link ActivityAction#PATTERN_MATCHED} search conditions
   */
  private Filter buildPatternMatchedActivityFilter(FilterTarget filterTarget, Long itemId) {
    return Filter.builder()
        ...
        .withCondition(
            FilterCondition.builder().eq(CRITERIA_OBJECT_ID, String.valueOf(itemId)).build())
        .withCondition(FilterCondition.builder()
            .eq(CRITERIA_ENTITY, Activity.ActivityEntityType.PATTERN.getValue()).build())
        .withCondition(
            FilterCondition.builder().eq(CRITERIA_ACTION, ActivityAction.PATTERN_MATCHED.getValue())
                .build())
        .build();
  }
}

...
/**
 * Abstract base for all domain events, supporting optional state snapshots and user context. Events are serialized to
 * JSON for API/message queue integration and can be published using Spring's {@code ApplicationEventPublisher}.
 *
...
public abstract class AbstractEvent<T> extends ApplicationEvent {
  ...
  /**
   * Returns whether this is a system event (internal processing, no user context). System events don't require activity
   * tracking as they represent automated processes.
   *
  ...
  /**
   * Returns whether this event should be published to RabbitMQ. Default implementation returns true for backward
   * compatibility. Override in subclasses to mark events as local-only.
   *

...
public class GetWidgetHandlerImpl implements GetWidgetHandler {
  ...
  public WidgetResource getWidget(Long widgetId, String[] attributes,
    ...
    if (unfilteredWidgetTypes.contains(widgetType) || isFilteredContentLoadAllowed(
      ...
      content = ofNullable(multilevelLoadContentStrategy.get(widgetType)).map(
          strategy -> strategy.loadContent(Lists.newArrayList(
  ...
  public Map<String, ?> getWidgetPreview(WidgetPreviewRQ previewRQ,
    ...
    if (CollectionUtils.isNotEmpty(previewRQ.getFilterIds())) {
      userFilter = getPermittedFilters(previewRQ.getFilterIds().toArray(Long[]::new),
          projectDetails, user);
    }
  ...
  public Iterable<Object> getOwnNames(ReportPortalUser.ProjectDetails projectDetails,
      ...
      ReportPortalUser user) {
    Page<Widget> own = widgetRepository.getOwn(
        ProjectFilter.of(filter, projectDetails.getProjectId()), pageable, user.getUsername());
    return PagedResourcesAssembler.pageConverter().apply(own.map(Widget::getName));
  }

  @Override
  public Iterable<WidgetResource> getShared(ReportPortalUser.ProjectDetails projectDetails,
      Pageable pageable, Filter filter,
      ReportPortalUser user) {
    Page<Widget> shared = widgetRepository.getShared(
  ...
  @Override
  public Iterable<WidgetResource> searchShared(ReportPortalUser.ProjectDetails projectDetails,
      Pageable pageable, Filter filter,
      ReportPortalUser user, String term) {
    Filter termFilter = Filter.builder()
        .withTarget(Widget.class)
        .withCondition(
            new FilterCondition(Operator.OR, Condition.CONTAINS, false, term, CRITERIA_NAME))
        .withCondition(
            new FilterCondition(Operator.OR, Condition.CONTAINS, false, term, CRITERIA_OWNER))
        .build();
    Page<Widget> shared = widgetRepository.getShared(
        ProjectFilter.of(new CompositeFilter(Operator.AND, filter, termFilter),

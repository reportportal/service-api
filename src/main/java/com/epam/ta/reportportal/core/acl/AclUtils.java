...
public class AclUtils {
    ...
    public static void monopolize(Acl acl, String projectName) {
        ...
        Optional<AclEntry> aclEntry = entries.stream()
                .filter(Preconditions.hasACLPermission(projectName, AclPermissions.READ)).findFirst();
    ...
    public static boolean isAllowedToDeleteWidget(Acl dashboardAcl, Acl widgetAcl, String modifier,
        ...
        return widgetAcl.getOwnerUserId().equals(modifier) ||
                !dashboardAcl.getOwnerUserId().equals(modifier) &&
    ...
    public static void isPossibleToRead(Acl acl, String userModifier, String projectName) {
        BusinessRule
                .expect(acl, or(Preconditions.isOwner(userModifier), Preconditions.isSharedTo(projectName)))
                .verify(ErrorType.UNABLE_MODIFY_SHARABLE_RESOURCE, Suppliers.formattedSupplier(
                        "User '{}' isn't owner of resource and resource isn't shared to project '{}'.", userModifier,
                        projectName));
    }
    ...
    public static void share(Acl acl, String projectName) {
        Set<AclEntry> entries = acl.getEntries();
        Optional<AclEntry> entry = entries.stream().filter(Preconditions.hasACLPermission(projectName, null))
                .findFirst();
        if (entry.isPresent()) {
    ...
    private static Predicate<Acl> hasProjectRole(Map<String, ProjectRole> userProjects, ProjectRole role) {
        return acl -> acl.getEntries().stream()
                .anyMatch(e -> userProjects.containsKey(e.getProjectId())
                        && userProjects.get(e.getProjectId()).compareTo(role) >= 0);
    }
}

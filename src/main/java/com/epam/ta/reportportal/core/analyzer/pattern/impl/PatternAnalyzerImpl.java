...
public class PatternAnalyzerImpl implements PatternAnalyzer {
  ...
  private List<Long> attachToPatternTemplate(Launch launch, PatternTemplate patternTemplate,
      List<Long> itemIds) {
    final List<Long> matchedIds = filterItems(launch, patternTemplate, itemIds);
    final List<PatternTemplateTestItemPojo> patternTemplateTestItems = convertToPojo(
        patternTemplate, matchedIds);
    patternTemplateRepository.saveInBatch(patternTemplateTestItems);
    publishEvents(patternTemplate, patternTemplateTestItems);
    return matchedIds;
  }

  private List<Long> filterItems(Launch launch, PatternTemplate patternTemplate,
      List<Long> itemIds) {
    final PatternAnalysisSelector patternAnalysisSelector = patternAnalysisSelectorMapping.get(
        patternTemplate.getTemplateType());
    return patternAnalysisSelector.selectItemsByPattern(launch.getId(), itemIds,

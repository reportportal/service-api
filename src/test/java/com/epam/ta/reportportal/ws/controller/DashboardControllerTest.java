...
class DashboardControllerTest extends BaseMvcTest {
	...
	void createDashboardPositive() throws Exception {
		...
		final MvcResult mvcResult = mockMvc.perform(post(DEFAULT_SLUG_KEY_BASE_URL + "/dashboard").with(token(oAuthHelper.getDefaultToken()))
	...
	void getAllDashboardsPositive() throws Exception {
		mockMvc.perform(get(DEFAULT_SLUG_KEY_BASE_URL + "/dashboard").with(token(oAuthHelper.getDefaultToken()))).andExpect(status().isOk());
	}

	@Test
	void getDashboardPositive() throws Exception {
		mockMvc.perform(get(DEFAULT_SLUG_KEY_BASE_URL + "/dashboard/17").with(token(oAuthHelper.getDefaultToken())))
	...
	void updateDashboardPositive() throws Exception {
		...
		mockMvc.perform(put(DEFAULT_SLUG_KEY_BASE_URL + "/dashboard/17").with(token(oAuthHelper.getDefaultToken()))
	...
	void deleteDashboardPositive() throws Exception {
		mockMvc.perform(delete(DEFAULT_SLUG_KEY_BASE_URL + "/dashboard/17").with(token(oAuthHelper.getDefaultToken())))
				.andExpect(status().isOk());
	}

	@Test
	void getSharedDashboardsNamesPositive() throws Exception {
		mockMvc.perform(get(DEFAULT_SLUG_KEY_BASE_URL + "/dashboard/shared").with(token(oAuthHelper.getDefaultToken())))
				.andExpect(status().isBadRequest());
	}

	@Test
	void removeWidget() throws Exception {
		mockMvc.perform(delete(DEFAULT_SLUG_KEY_BASE_URL + "/dashboard/18/10").with(token(oAuthHelper.getDefaultToken())))
				.andExpect(status().isOk());
	}
	...
	void addWidget() throws Exception {
		...
		mockMvc.perform(put(DEFAULT_SLUG_KEY_BASE_URL + "/dashboard/17/add").with(token(oAuthHelper.getDefaultToken()))

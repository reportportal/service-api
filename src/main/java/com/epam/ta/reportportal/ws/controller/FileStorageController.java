...
public class FileStorageController {
	...
	public void getFile(@PathVariable String projectKey, @PathVariable("dataId") Long dataId, HttpServletResponse response,
			@AuthenticationPrincipal ReportPortalUser user) {
		toResponse(response, getFileHandler.loadFileById(dataId, projectExtractor.extractProjectDetails(user, projectKey)));
	}
	...
	public void getUserPhoto(@PathVariable String projectKey, @RequestParam(value = "id") String username,
			...
			@AuthenticationPrincipal ReportPortalUser user) {
		BinaryData userPhoto = getFileHandler.getUserPhoto(EntityUtils.normalizeId(username), user, projectKey, loadThumbnail);

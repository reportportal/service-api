...
public class DemoLogsService {
  ...
  public DemoLogsService(
      @Value("${rp.environment.variable.demo.attachment.probability}") int attachmentProbability,
  ...
  public List<Log> generateLaunchLogs(int count, String launchUUid, StatusEnum status) {
    ...
    logService.saveLogMessageList(logs, launch.getId());
  ...
  public List<Log> generateItemLogs(int count, Long projectId, String itemUuid, StatusEnum status) {
    ...
    if (FAILED.equals(status)) {
      List<String> errors = ContentUtils.getErrorLogs();
      logs.addAll(errors.stream().map(msg -> getLog(projectId, testItem, errorLevel(), msg))
          .collect(toList()));
    }
    logRepository.saveAll(logs);
    logService.saveLogMessageList(logs, testItem.getLaunchId());
  ...
  private void createAttachments(List<Log> logs, Long projectId, Long launchId, Long itemId,
    ...
    logs.forEach(it -> {
      if (ERROR.toInt() >= it.getLogLevel()) {
        ...
        if (!binaryDataAttached.getValue() && ContentUtils.getWithProbability(

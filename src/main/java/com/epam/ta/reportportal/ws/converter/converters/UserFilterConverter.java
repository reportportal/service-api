...
public final class UserFilterConverter {
  ...
  private static UserFilterResource buildFilterResource(UserFilter filter) {
    ...
    ofNullable(filter.getFilterCondition()).ifPresent(fcs -> userFilterResource.setConditions(
        fcs.stream().map(UserFilterConverter.TO_FILTER_CONDITION).collect(toSet())));
    ofNullable(filter.getFilterSorts()).ifPresent(fs -> userFilterResource.setOrders(
        fs.stream().map(UserFilterConverter.TO_FILTER_ORDER).collect(toList())));

...
values (1, 'uuid', 1, 'superadmin', 'test launch', 'desc', now() - interval '9 minute', now(), 1, now(), 'DEFAULT', 'FAILED', false);

-- Second launch
insert into launch(id, uuid, project_id, owner, name, description, start_time, end_time, number, last_modified, mode, status, has_retries)
values (2, 'uuid2', 1, 'superadmin', 'test launch', 'desc', now() - interval '11 minute', now(), 2, now(), 'DEFAULT', 'FAILED', false);

...
public class ElasticLogService implements LogService {
    ...
    public Map<Long, List<IndexLog>> findAllIndexUnderTestItemByLaunchIdAndTestItemIdsAndLogLevelGte(Long launchId, List<Long> itemIds, int logLevel) {
        ...
        Map<Long, List<IndexLog>> indexLogMap = logRepository.findAllIndexUnderTestItemByLaunchIdAndTestItemIdsAndLogLevelGte(launchId, itemIds, logLevel);
    ...
    private Map<Long, List<IndexLog>> wrapLogsWithLogMessages(Long projectId, Map<Long, List<IndexLog>> indexLogMap) {
        ...
        if (indexLogMap != null && indexLogMap.size() > 0) {
            ...
            Map<Long, LogMessage> logMessageMap = elasticSearchClient.getLogMessagesByProjectIdAndIds(projectId, logIds);
    ...
    public List<String> findMessagesByLaunchIdAndItemIdAndPathAndLevelGte(Long launchId, Long itemId, String path, Integer level) {
        Long projectId = launchRepository.findById(launchId).map(Launch::getProjectId).orElseThrow();
        List<Long> logIds = logRepository.findIdsByLaunchIdAndItemIdAndPathAndLevelGte(launchId, itemId, path, level);

...
public class LaunchPatternAnalyzerImpl implements LaunchPatternAnalyzer {
  ...
  private final ItemsPatternAnalyzer itemsPatternAnalyzer;

  @Autowired
  public LaunchPatternAnalyzerImpl(
      @Value("${rp.environment.variable.pattern-analysis.batch-size}") Integer batchSize,
      TestItemRepository testItemRepository, TaskExecutor patternAnalysisTaskExecutor,
      PatternConditionProviderChain patternConditionProviderChain,
      AnalyzerStatusCache analyzerStatusCache, ItemsPatternAnalyzer itemsPatternAnalyzer) {
    ...
    this.itemsPatternAnalyzer = itemsPatternAnalyzer;
  }
  ...
  public void analyzeLaunch(Launch launch, Set<AnalyzeItemsMode> analyzeModes) {
    ...
    patternAnalysisTaskExecutor.execute(() -> {
      try {
        final ConvertibleCondition itemCondition = buildItemsCondition(analyzeModes);
        analyze(launch, itemCondition);
      } catch (Exception e) {
  ...
  private void analyze(Launch launch, ConvertibleCondition itemCondition) {
    ...
    while (CollectionUtils.isNotEmpty(itemIds)) {
      itemsPatternAnalyzer.analyzeItems(launch.getProjectId(), launch.getId(), itemIds);
      offset += itemIds.size();

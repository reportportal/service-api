...
public interface UpdatePluginHandler {
  ...
  /**
   * Updates plugin state. If 'enabled == true', plugin file will be downloaded from the {@link DataStore} (if not
   * exists in the plugins' root path) and loaded in the memory. If 'enabled == false', plugin will be unloaded from the
   * memory
   *

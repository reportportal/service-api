...
public interface TmsTestFolderRepository extends ReportPortalRepository<TmsTestFolder, Long> {
   ...
   */
  @Query(
      "SELECT new com.epam.reportportal.base.infrastructure.persistence.entity.tms."
   ...
   */
  @Query("SELECT new com.epam.reportportal.base.infrastructure.persistence.entity.tms."
  ...
  @Query(value = """
      WITH RECURSIVE ids AS (
        SELECT id, parent_id FROM tms_test_folder WHERE id IN :folderIds AND project_id = :projectId
        UNION ALL
        SELECT tf.id, tf.parent_id FROM tms_test_folder tf JOIN ids ON tf.id = ids.parent_id
      )
      SELECT id FROM ids
      """, nativeQuery = true)

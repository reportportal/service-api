...
RUN apt-get update -qq && apt-get install -qq -y wget fontconfig && \
	echo 'exec java ${JAVA_OPTS} -jar service-api-5.3.1-exec.jar' > /start.sh && chmod +x /start.sh && \
	wget -q https://dl.bintray.com/epam/reportportal/com/epam/reportportal/service-api/5.3.1/service-api-5.3.1-exec.jar

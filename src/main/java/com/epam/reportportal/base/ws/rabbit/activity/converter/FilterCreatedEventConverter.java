...
import com.epam.reportportal.base.core.events.domain.FilterCreatedEvent;
...
/**
 * Converter for FilterCreatedEvent to Activity.
 */
@Component
public class FilterCreatedEventConverter implements EventToActivityConverter<FilterCreatedEvent> {

  @Override
  public Activity convert(FilterCreatedEvent event) {
    return new ActivityBuilder()
        .addCreatedAt(event.getOccurredAt())
        .addAction(EventAction.CREATE)
        .addEventName(ActivityAction.CREATE_FILTER.getValue())
        .addPriority(EventPriority.LOW)
        .addObjectId(event.getUserFilterActivityResource().getId())
        .addObjectName(event.getUserFilterActivityResource().getName())
        .addObjectType(EventObject.FILTER)
        .addProjectId(event.getUserFilterActivityResource().getProjectId())
  ...
  @Override
  public Class<FilterCreatedEvent> getEventClass() {
    return FilterCreatedEvent.class;
  }
}

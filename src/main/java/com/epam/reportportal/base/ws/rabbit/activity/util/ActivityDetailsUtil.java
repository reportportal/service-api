...
package com.epam.reportportal.base.ws.rabbit.activity.util;
...
/**
 * Utilities for building {@link HistoryField} instances describing changes of entity attributes for activity logging.
 *
...
public class ActivityDetailsUtil {
  ...
  /**
   * Returns the appropriate subject name for an event based on whether it's a system event. For system events, uses the
   * event source; for user events, uses the user login.
   *
  ...
  /**
   * Returns the appropriate subject type for an event based on whether it's a system event. System events use
   * APPLICATION subject type; user events use USER subject type.
   *
   * @param event The domain event

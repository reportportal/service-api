...
package com.epam.reportportal.base.core.configs.security;
...
/**
 * Custom authentication manager resolver that selects between API key and JWT authentication based on the request's
...
public class CustomAuthenticationManagerResolver implements AuthenticationManagerResolver<HttpServletRequest> {
  ...
  /**
   * Constructs a CustomAuthenticationManagerResolver with the provided API key authentication provider and JWT issuer
  ...
  /**
   * Resolves the appropriate AuthenticationManager based on the request's Authorization header. If the token is a JWT,
   * it uses the JWT resolver; otherwise, it uses the API key manager.
   *
